...
__global__ void kernelGEMVtransposed(float* Z, const float* Atr, const float* X, const float* Y,
	...
	if (gy < matsize) {
		...
		register unsigned last_elem = matsize - (ntiles - 1) * BLOCK_SIZE;
		register float sum = 0.0f;
		ntiles = 2;
		for (register unsigned tile = 0; tile < ntiles - 1; ++tile) {
			...
			for (register unsigned k = 0; k < BLOCK_SIZE; ++k) {
				sum += *ptr * s_X[k];
				ptr += pitch_in_elem;
			}
			__syncthreads();
...
#if 0
		// last tile
		register unsigned tile = ntiles - 1;
		if (threadIdx.y < last_elem) {
			register unsigned index = tile * BLOCK_SIZE + threadIdx.y;
			s_X[threadIdx.y] = X[index];
		}
		__syncthreads();
		register unsigned idxA = (tile * BLOCK_SIZE) * pitch_in_elem + gy;
		const float* ptr = Atr + idxA;
		for (register unsigned k = 0; k < last_elem; ++k) {
			sum += *ptr * s_X[k];
			ptr += pitch_in_elem;
		}
#endif
