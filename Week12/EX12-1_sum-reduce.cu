...
__global__ void kernelSum(const float* pData) {
	extern __shared__ float s_data[];
	register unsigned gx = threadIdx.x + blockIdx.x * blockDim.x;
...
int main(const int argc, const char* argv[]) {
	...
	kernelSum << < dimGrid, dimBlock, dimBlock.x * sizeof(float) >> > (dev_vecData);
