...
// input parameters
const unsigned BLOCK_SIZE = 1024; // fixed for simplicity
unsigned NUM = 256 * 1024 * 1024; // num of samplings

// CUDA kernel function
__global__ void kernelEvenOdd(float* dst, const float* src, unsigned num, unsigned half) {
	...
	if (tx % 2 == 0) {
		...
		s_dst[tx / 2 + BLOCK_SIZE / 2] = s_src[tx];
	}
	__syncthreads();
...
int main(const int argc, const char* argv[]) {
	...
	kernelEvenOdd << < dimGrid, dimBlock >> > (dev_dst, dev_src, NUM, NUM / 2);
