...
__global__ void kernelHist(const unsigned* img, unsigned num, unsigned* hist) {
	...
	if (i < num) {
		...
		atomicAdd(&(hist[((pixelVal >> 0) & 0xFF) / 8]), 1);
		atomicAdd(&(hist[((pixelVal >> 8) & 0xFF) / 8]), 1);
		atomicAdd(&(hist[((pixelVal >> 16) & 0xFF) / 8]), 1);
		atomicAdd(&(hist[((pixelVal >> 24) & 0xFF) / 8]), 1);
	}
}
...
int main(const int argc, const char* argv[]) {
	...
	kernelHist << < dimGrid, dimBlock >> > (reinterpret_cast<unsigned*>(dev_image), (image_width * image_height / 4),
		dev_vecHist);
