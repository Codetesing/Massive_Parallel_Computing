...
int my_binary_search(unsigned* pData, int first, int last, unsigned target) {
	int index;
	if (first > last) {
		index = -1;
	}
		...
		if (target == pData[mid]) {
			...
			index = my_binary_search(pData, first, mid - 1, target);
		}
		else {
			index = my_binary_search(pData, mid + 1, last, target);
		}
	}
	return index;
}
...
int main(const int argc, const char* argv[]) {
	...
	int index = my_binary_search(vecData, 0, num, targetValue);
	ELAPSED_TIME_END(0);
	// check the result
	if (index == -1) {
