...
int main(const int argc, const char* argv[]) {
	...
	case 4:
		num = procArg(argv[0], argv[1], 1024);
		BLOCK_SIZE = procArg(argv[0], argv[2], 32, 1024);
		count = procArg(argv[0], argv[3], 32);
		break;
	default:
	...
	dim3 dimGrid(div_up(num, count * dimBlock.x), 1, 1);
	// CUDA_PRINT_CONFIG( num );
	ELAPSED_TIME_BEGIN(0);
	kernelSearch << < dimGrid, dimBlock >> > (dev_vecData, num, targetValue, dev_vecIndex, sizeIndex, count);
