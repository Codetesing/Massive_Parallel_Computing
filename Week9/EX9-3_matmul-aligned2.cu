...
__global__ void kernelMatMul(float* C, const float* A, const float* B, unsigned matsize,
	size_t pitch_in_elem, unsigned TILE_WIDTH) {
...
int main(const int argc, const char* argv[]) {
	...
	if (matsize % 32 != 0) {
		printf("%s: matsize=%d should be multiples of 32\n", argv[0], matsize);
		exit(EXIT_FAILURE); // EINVAL: invalid argument
	}
	if (matsize % TILE_WIDTH != 0) {
		printf("%s: matsize=%d should be multiples of TILE_WIDTH=%d\n", argv[0], matsize, TILE_WIDTH);
		exit(EXIT_FAILURE); // EINVAL: invalid argument
	}
	// host-side data
	float* matA = nullptr;
