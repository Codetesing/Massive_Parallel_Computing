...
__global__ void kernel_matadd(float* c, const float* a, const float* b, unsigned nrow, unsigned ncol) {
	unsigned row = blockIdx.x * blockDim.x + threadIdx.x; // CUDA-provided index
	unsigned col = blockIdx.y * blockDim.y + threadIdx.y; // CUDA-provided index
...
int main(const int argc, const char* argv[]) {
	...
	dim3 dimGrid((nrow + dimBlock.x - 1) / dimBlock.x, (ncol + dimBlock.y - 1) / dimBlock.y, 1);
	CUDA_PRINT_CONFIG_2D(nrow, ncol);
